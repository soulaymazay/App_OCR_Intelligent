...
def create_purchase_invoice_from_log(log_name: str) -> str:
    ...
    # ── Attachement du fichier source OCR ─────────────────────────────
    try:
        if hasattr(log, 'file_url') and log.file_url:
            from ocr_intelligent.api.auto_create_document import attacher_copie_originale
            result = attacher_copie_originale("Purchase Invoice", inv.name, log.file_url)
            if result and result.get("success"):
                frappe.logger().info(f"[OCR] Fichier source attaché à Purchase Invoice {inv.name}")
    except Exception as e:
        frappe.logger().error(f"[OCR] Erreur attachement fichier à Purchase Invoice: {str(e)}")
...
def create_payment_entry_from_invoice(invoice_name: str, log_name: str = None) -> str:
    ...
    if log_name:
        ...
        try:
            ...
            if hasattr(log, 'file_url') and log.file_url:
                from ocr_intelligent.api.auto_create_document import attacher_copie_originale
                result = attacher_copie_originale("Payment Entry", pe.name, log.file_url)
                if result and result.get("success"):
                    frappe.logger().info(f"[OCR] Fichier source attaché à Payment Entry {pe.name}")
        except Exception as e:
            frappe.logger().error(f"[OCR] Erreur attachement fichier à Payment Entry: {str(e)}")
