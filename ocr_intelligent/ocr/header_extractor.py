...
TYPE_PATTERNS = {
    ...
    "facture_vente": [
        r"\bfacture\s+de\s+vente\b",          # label explicite
        r"\bfacture\s+client\b",               # label explicite
        r"\bfacture\s+(?:de\s+)?v(?:ente)?[-\s]?\d*\b",  # facture vente 001
        r"^\s*vendeur\b",                      # "Vendeur" en-tête (avec ou sans :)
        r"\bvendu\s+[àa]\b",                   # "Vendu à :"
        r"\bsold\s+to\b",                      # "Sold to:"
    ],
    "cheque": [
        r"\bch[eè]que\b", r"\bpayez\s+contre\b", r"\bpayable\s+[àa]\b",
        r"\bcinq\s+mille\b", r"\bmille\s+euro\b", r"\bbillet\b",
        r"\border\s+of\b", r"\bpay\s+to\b",
        # Patterns additionnels pour robustesse OCR
        r"\bstb\s+bank\b", r"\bbiat\s+bank\b", r"\bbna\s+bank\b",
        r"\bmontant\s+max\s*[:\-]?\s*\d+\s*dt\b",
        r"\bdate\s+d['\']?expiration\b",
        r"\bpayez\s+contre\s+ce\b",
        r"\btitulaire\s+du\s+compte\b",
        r"\bsignature\s*\(\s*s\s*\)",
        r"\bch[ée]que\s*n[°o]?\s*[:\-]?\s*\d{6,}",
        # Patterns numériques typiques des chèques tunisiens
        r"\b\d{6,7}\s+\w+\s+dt\b",  # "30000 DT"
        r"\b10\s+500\s+002\s+\d{8,}\b",  # Format RIB STB
    ],
    "traite": [
        ...
        r"\b[eé]ch[eé]ance\s*[:\-]?\s*\d{2}[/\-]\d{2}[/\-]\d{4}",  # Date échéance typique des traites
        # Patterns additionnels pour robustesse OCR
        r"\brepublique\s+tunisienne\b.*\blettre\b",  # En-tête typique traite tunisienne
        r"\bc['\']?est\s+soixante\s+sept\s+dinars\b",  # Montant en lettres
        r"\bsept\s+dinars\b",
        r"\bdomiciliation\s*[:\-]?\s*stb\b",
        r"\boleo\s+techno\b",  # Entreprises typiques sur traites
        r"\b\d{11,12}\b.*\bsousse\b",  # Code barre + ville
        r"\bordre\s+de\s+paiement\b",
    ],
}

# Patterns clés : si présents, identifient de façon certaine le type de document
# Ces patterns ont une priorité absolue sur les patterns génériques
PATTERNS_CLES = {
    "bon_livraison": [
    ...
    "traite": [
        ...
        r"\beffet\s*de\s*commerce\b",
        r"\beffet\s*n[°o]",
        # Patterns robustes pour traites tunisiennes
        r"\brepublique\s+tunisienne.*lettre",
        r"\bdomiciliataire\s*[:\-]",
        r"\btireur\s*[:\-]",
        r"\btiré\s*[:\-]",
        r"\bordre\s+de\s+paiement\s*[:\-]?\s*c\s*['\"]?p",
    ],
    "cheque": [
        r"\bch[eè]que\s*n[°o]",
        r"\bpayez\s+contre\s+ce\s+ch[eè]que\b",
        r"\bch[eè]que\s*[:\-]?\s*\d+",
        # Patterns robustes pour chèques tunisiens
        r"\bmontant\s+max\s*[:\-]?\s*\d+\s*dt",
        r"\bdate\s+d['\']?expiration\s*[:\-]",
        r"\btitulaire\s+du\s+compte\b",
        r"\bstb\s+bank\b",
        r"\bpayez\s+contre\s+ce\b",
    ],
    ...
    "bon_commande": [
        r"\bbon\s+de\s+commande\s*n[°o]",
        r"\bpurchase\s+order\s*n[°o]",
    ],
    "devis": [
        r"\bdevis\s*n[°o]",
        r"\bquotation\s*n[°o]",
...
def detecter_type_document(texte: str) -> str:
    ...
    Stratégie améliorée :
    1. Vérifier d'abord les patterns clés (très spécifiques)
    2. Si un pattern clé est trouvé, retourner immédiatement ce type
    3. Sinon, utiliser le scoring normal avec tous les patterns
    """
    texte_lower = texte.lower()
    
    # ── Étape 1 : Vérifier les patterns clés (priorité absolue) ──────
    for type_doc, patterns_cles in PATTERNS_CLES.items():
        for pattern in patterns_cles:
            if re.search(pattern, texte_lower, re.IGNORECASE | re.MULTILINE):
                # Pattern clé trouvé : identification certaine
                return type_doc
    
    # ── Étape 2 : Scoring normal si aucun pattern clé trouvé ─────────
    ...
    # "facture_vente" est plus spécifique que "facture" :
    # si au moins un de ses patterns correspond, il a la priorité absolue
    # afin d'éviter que le pattern générique \bfacture\b ne l'emporte.
    if scores.get("facture_vente", 0) > 0:
        ...
        if match:
            ...
            if champ == "numero_facture":
                ...
                if not re.search(r'\d', valeur):
                    continue
                # Rejeter si c'est uniquement une année sur 4 chiffres (1900-2099)
                if re.fullmatch(r'(?:19|20)\d{2}', valeur):
                    continue
            ...
            # Rejeter les numéros fiscaux tunisiens confondus avec montant TVA
            # ex: "TVA: 1314124ZAM000" → capture "1314124" mais suivi de "ZAM000"
            if champ == "tva":
                # Trouver la fin du token numérique dans le texte original
                g1_end = match.end(1)
                # Scanner les caractères suivants jusqu'à un séparateur
                tail = texte[g1_end:g1_end + 8]
                tail_stripped = re.sub(r'^\s*', '', tail)  # ignorer espaces immédiats
                if tail_stripped and re.match(r'[A-Z]{2,}', tail_stripped):
                    # Suivi de lettres majuscules → numéro fiscal, pas un montant
                    continue
