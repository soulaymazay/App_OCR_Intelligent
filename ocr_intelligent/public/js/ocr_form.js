// ocr_form.js
// v10 — Fix définitif : frm._ocr_source_file_url prioritaire + propagation polling + Payment Entry
...
const OCR_DIALOG_FIELDS_CHEQUE = [
    { ocr: "numero_cheque",    frappe: "reference_no",          label: "N° Chèque / Référence",  type: "Data",     sensitivity: 1 },
    { ocr: "date_cheque",      frappe: "reference_date",        label: "Date du Chèque",          type: "Date",     sensitivity: 2 },
    { ocr: "amount",           frappe: "paid_amount",           label: "Montant (TND)",           type: "Currency", sensitivity: 1 },
    { ocr: "beneficiaire",     frappe: "party",                 label: "À l'ordre de",            type: "Data",     sensitivity: 2 },
    { ocr: "banque",           frappe: "bank",                  label: "Banque",                  type: "Data",     sensitivity: 1 },
    { ocr: "titulaire_compte", frappe: "account_holder_name",   label: "Titulaire du Compte",     type: "Data",     sensitivity: 3 },
    { ocr: "rib",              frappe: "bank_account",          label: "RIB / Compte",            type: "Data",     sensitivity: 2, aliases: ["bank_account", "rib"] },
];
...
const OCR_DIALOG_FIELDS_TRAITE = [
    ...
    {
        ocr: "rib",
        frappe: "custom_rib_tire",
        label: "RIB du Tiré",
        type: "Data",
        sensitivity: 2,
        aliases: ["custom_rib_tire", "rib", "bank_account"],
    },
];
...
const OCRForm = {
    _add_button(frm) {
    // Frappe recrée les custom_buttons à chaque refresh — pas besoin de guard
            ...
            fields: [
                {
                    ...
                    options: `<div style="background:var(--bg-light-blue);
                        ...
                        <b>Formats acceptés :</b> PDF · PNG · JPG · JPEG · TIFF · BMP · XLSX · SVG
                    </div>`
                },
                {
                    fieldtype : "Attach",
    ...
    _lancer(frm, file_url) {
        ...
        // ── Mémoriser le file_url source pour l'attachement ultérieur ──
        frm._ocr_source_file_url = file_url;
        ...
        frappe.call({
            ...
            callback(r) {
                ...
                // ── Injecter le file_url source dans result si absent ──
                if (!result.file_url && !result._source_file_url) {
                    result._source_file_url = file_url;
                }
                if (result.async && result.job_id) {
                    // ── Stocker le file_url source sur le job aussi ──
                    result._source_file_url = result._source_file_url || file_url;
                    frappe.show_progress(__("OCR en cours..."), 40, 100, __("Analyse du document..."));
                    OCRForm._poll_ocr(frm, result.job_id, Date.now(), 0, file_url);
                } else {
                    frappe.hide_progress();
    ...
    // ── ÉTAPE 2b : Polling ─────────────────────────────────────
    // CORRECTION : file_url_source ajouté en paramètre pour l'injecter dans result
    _poll_ocr(frm, job_id, startTime, retryCount = 0, file_url_source = "") {
        ...
        frappe.call({
            ...
            callback(r) {
                ...
                if (s.status === "termine") {
                    ...
                    // ── CORRECTION : injecter file_url_source si absent dans le résultat ──
                    if (!result.file_url && !result._source_file_url && file_url_source) {
                        result._source_file_url = file_url_source;
                    }
                    OCRForm._traiter(frm, result);
                    ...
                    setTimeout(() => OCRForm._poll_ocr(frm, job_id, startTime, 0, file_url_source), INTERVAL_MS);
                }
            },
            error() {
                ...
                setTimeout(() => OCRForm._poll_ocr(frm, job_id, startTime, retryCount + 1, file_url_source), backoff);
            }
        });
    },
    ...
    _traiter(frm, result) {
        ...
        if (!result.success) {
            ...
            const typeDoc   = result.type_document || "inconnu";
            const typeLabel = typeLabels[typeDoc] || typeDoc;
            const msgBase   = (result.erreur || __("Erreur inconnue")).replace(/\n/g, "<br>");
            const tagColor  = typeDoc === "inconnu" ? "grey" : "red";
            const msgFinal  = `<div style="margin-bottom:10px">
                <span style="background:${tagColor === "red" ? "#ffe0e0" : "#f0f0f0"};
                             color:${tagColor === "red" ? "#c0392b" : "#555"};
                             padding:3px 10px; border-radius:4px; font-weight:bold; font-size:13px;">
                    ${frappe.utils.escape_html(typeLabel)}
                </span>
            </div>
            <div>${msgBase}</div>`;
            frappe.msgprint({
                title    : result.titre || __("Document refusé"),
                message  : msgFinal,
                indicator: "red"
            });
            return;
    ...
    _dialog_doublon(frm, result) {
        ...
        const msgDoublon = `<b>⚠️ ${__("Doublon détecté")}</b><br><br>
        ...
        const d    = new frappe.ui.Dialog({
            ...
            primary_action()   { d.hide(); frappe.set_route("Form", frm.doctype, name); }
        });
        d.show();
    },
    ...
    _dialog_validation(frm, champs, result) {
        ...
        const uncertain = (result && result.uncertain_fields) || [];
        ...
        const uncertain_frappe = new Set();
        for (const def of defs) {
            const ocr_keys = [def.ocr, ...(def.aliases || [])];
            if (ocr_keys.some(k => uncertain.includes(k))) {
                uncertain_frappe.add(def.frappe);
            }
        }
        ...
        for (const def of defs) {
            ...
            const is_uncertain = uncertain_frappe.has(def.frappe);
            const sensitivity  = def.sensitivity || 2;

            if (is_uncertain) {
                field.description = __("⚠ Confiance faible — vérifiez cette valeur");
            } else if (!val && sensitivity === 1) {
                field.description = __("⛔ Champ obligatoire — non détecté, saisie manuelle requise");
            } else if (val && sensitivity === 1) {
                field.description = __("✓ Extrait avec haute confiance");
            }
    ...
    _appliquer_au_formulaire(frm, vals, enregistrer, result) {
        ...
        if (frm.doctype === "Payment Entry") {
            ...
            return type_doc === "traite"
                ? OCRForm._appliquer_traite(frm, vals, enregistrer, result)
                : OCRForm._appliquer_cheque(frm, vals, enregistrer, result);
        }
            ...
            .then(() => {
                ...
                if (enregistrer) {
                    return frm.save().then(() => {
                        ...
                        // ── Lier l'OCR Document à la facture sauvegardée ──
                        const ocr_doc_id = result && result.ocr_document_id;
                        if (ocr_doc_id && frm.doc.name) {
                            frappe.call({
                        ...
                        // ── Attacher le fichier original ──
                        OCRForm._attacher_fichier_original(frm, result);
                    });
                }
    ...
    // ── Application chèque ─────────────────────────────────────
    _appliquer_cheque(frm, vals, enregistrer, result) {
        vals = (vals && typeof vals === "object") ? vals : {};
        return frm.set_value("payment_type", "Pay")
            ...
            .then(() => {
                ...
                if (vals.account_holder_name)    simple.account_holder_name   = vals.account_holder_name;
                if (vals.bank_account)           simple.bank_account          = vals.bank_account;
    ...
    // ── Application traite ─────────────────────────────────────
    _appliquer_traite(frm, vals, enregistrer, result) {
        vals = (vals && typeof vals === "object") ? vals : {};
        return frm.set_value("payment_type", "Pay")
            .then(() => new Promise(r => frappe.after_ajax(r)))
            .then(() => frm.set_value("party_type", "Supplier"))
            ...
            .then(() => {
                ...
                if (vals.custom_rib_tire)        simple.custom_rib_tire       = vals.custom_rib_tire;
                if (!Object.keys(simple).length) return;
                return frm.set_value(simple);
            })
            .then(() => {
    ...
    // ── Attacher le fichier original ───────────────────────────
    // Résout toutes les clés possibles où file_url peut se trouver dans result,
    // avec fallback sur frm._ocr_source_file_url mémorisé lors du _lancer().
    _attacher_fichier_original(frm, result) {
        // PRIORITÉ : frm._ocr_source_file_url mémorisé dès l'upload dans _lancer()
        // car result.file_url peut être null même si la clé existe dans le résultat
        const file_url = frm._ocr_source_file_url || (result && (
            result.file_url          ||   // clé principale pipeline (peut être null)
            result._source_file_url  ||   // clé de secours injectée par _lancer/_poll
            result.source_file_url   ||   // variante sans underscore
            result.original_file_url ||   // autre variante possible
            result.fichier           ||   // clé brute upload
            result.attachment_url         // encore une variante
        )) || null;

        const docname = frm.doc.name || frm.docname;

        if (!file_url) {
            console.warn("OCR _attacher_fichier_original : aucun file_url trouvé dans result", result);
            return;
        }
        if (!docname) {
            console.warn("OCR _attacher_fichier_original : docname introuvable");
            return;
        }

        frappe.call({
            method  : "ocr_intelligent.api.auto_create_document.attacher_copie_originale",
            args    : { doctype: frm.doctype, docname, file_url },
            freeze  : false,
            callback(r) {
                ...
                if (res && res.success) {
                    ...
                    // Afficher un avertissement si l'attachement a échoué côté serveur
                    const err_msg = (res && res.error) || __("Échec de l'attachement du fichier original.");
                    console.warn("OCR attacher_copie_originale :", err_msg);
                    frappe.show_alert({ message: __("⚠ Fichier non attaché : {0}", [err_msg]), indicator: "orange" }, 5);
                }
            },
            error(err) {
                console.error("OCR attacher_copie_originale RPC error:", err);
                frappe.show_alert({ message: __("⚠ Erreur lors de l'attachement du fichier."), indicator: "orange" }, 5);
            }
        });
    },
...
// NOUVEAU — supprimer le guard _ocr_init, laisser refresh le gérer
["Purchase Invoice", "Payment Entry", "OCR Document"].forEach(dt => {
    frappe.ui.form.on(dt, {
        refresh(frm)            { OCRForm._add_button(frm); },
        onload_post_render(frm) { OCRForm._add_button(frm); },
    });
});
frappe.ui.form.on("Payment Entry",    { refresh(frm) { OCRForm._add_button(frm); } });
frappe.ui.form.on("OCR Document",     { refresh(frm) { OCRForm._add_button(frm); } });